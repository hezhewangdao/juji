...
本项目是一个响应式网站，采用"移动端优先"(Mobile First)的设计策略，使用 Tailwind CSS 实现一套代码适配多端(PC + H5 Mobile)。网站主要用于分享中国古典诗词名句，提供每日一句推荐、名句搜索和分享功能。
...
├── public/                       # 静态资源
│   ├── images/
│   │   ├── desktop-bg.png        # PC端高清大图
│   │   ├── mobile-bg.png         # 移动端竖屏裁切图
│   │   ├── default_avatar.png    # 默认用户头像 (512×512)
│   │   ├── default_book.png      # 默认书籍封面 (600×900)
│   │   ├── paper.png             # 海报背景-纸短情长
│   │   ├── shadow.png            # 海报背景-光影斑驳
│   │   ├── rain.png              # 海报背景-雨夜微醺
│   │   ├── sky.png               # 海报背景-旷野长空
│   │   ├── ink.png               # 海报背景-墨染流年
│   │   └── warm.png              # 海报背景-温暖一隅
...
网站采用传统的 HTML 页面样式，而非全屏布局。主要页面元素包括：

1. **顶部搜索栏** - 用于搜索诗词名句
2. **每日一句区域** - 展示精选名句，背景使用响应式图片（PC端使用desktop-bg.png，移动端使用mobile-bg.png）
3. **句子列表区域** - 展示所有诗词句子，采用瀑布流布局

### 设计特点

- 采用"移动端优先"的设计策略
- 使用 Tailwind CSS 实现响应式布局
- 每日一句背景区域适度大小，不占据整个视口
- 海报分享功能作为交互式组件，仅在用户点击分享时显示
...
| 图片名称 | 用途 | 尺寸 | 比例 |
|---------|------|------|------|
| default_avatar.png | 用户默认头像 | 512×512 | 1:1 |
| default_book.png | 书籍默认封面 | 600×900 | 2:3 |
| desktop-bg.png | PC端背景图 | - | - |
| mobile-bg.png | 移动端背景图 | - | - |
| paper.png | 海报背景-纸短情长 | - | 9:16 |
| shadow.png | 海报背景-光影斑驳 | - | 9:16 |
| rain.png | 海报背景-雨夜微醺 | - | 9:16 |
| sky.png | 海报背景-旷野长空 | - | 9:16 |
| ink.png | 海报背景-墨染流年 | - | 9:16 |
| warm.png | 海报背景-温暖一隅 | - | 9:16 |

所有图片均支持自适应调整，在不同设备上都能良好显示。
